...
| 20 | [Week packs](loops/20-week-packs.md) | 10-week path audit + one print-ready, double-sided A4 PDF per week (`/home/path/<week>/pack` → `npm run packs:build` → private `packs` bucket → `/home/path/<week>/pdf`); three week-10 fit lessons drafted (`generated`) | open-pr |
