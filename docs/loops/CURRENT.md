...
- **Task:** 6 / 10 done — `pulse-search.v1` + `pulse-digest.v1`, `src/lib/pulse/generate.ts` (web_search_20260209 max_uses 8 + allowed_domains, pause_turn resume, source enforcement), `pulse:generate --dry-run` → `NO API CREDIT — fixture digest` (3 sourced stories → `.eval/pulse-2026-08-24.json`); next: cron route + vercel.json
...
- **Next action:** `GET /api/cron/pulse` (Bearer `CRON_SECRET` check → 401; `resolveChatMode()` fixture branch; `storeDigest`), `vercel.json` crons `0 6 * * 1`, `maxDuration`.
