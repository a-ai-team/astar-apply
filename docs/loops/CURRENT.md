...
- **Task:** 3 / 10 done — `scripts/firms/author.ts` (Opus 5 structured, fixture branch prints `NO API CREDIT — fixture questions`) + 14 hand-written question fixtures (210 Qs, seeded `generated`); next: firm pages + timeline + filters + drill bridge
...
- **Next action:** `src/lib/firms/queries.ts`, `/home/interviews/firms` grid, `/home/interviews/firms/[slug]` (dossier, `ProcessTimeline`, `FirmQuestionList` filters, guidance accordion, Practise this → `startDrillFor(firmQuestionId)`); extend `getTurns`/`getInterviewQuestions` for `firm_question_id`.
