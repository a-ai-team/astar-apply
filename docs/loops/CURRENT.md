...
- **Task:** 1 / 10 — migration 0006 applied (5 tables, user_stats + user_activity_days views, search_content(), tsv columns); db:check 31/31
...
- **Next action:** task 2 `src/lib/practice/srs.ts` (ts-fsrs 5.4.1 wrapper + streak/mastery) + vitest. Then read the Loop 04 retro (`flashcardBack()` in `src/lib/content/question-schema.ts`; question rows carry `review_note`/`generated_by`/`prompt_version`; `difficultyShares()` in `src/lib/content/generate/checks.ts`; RLS serves only `approved`), then `docs/loops/05-practice.md` + CONTRACTS.md (tables `flashcards`, `reviews`, `card_state`, `attempts`, `lesson_progress` in migration 0006); do its "Research at start"; branch `feat/practice`; task 1. Seeds before e2e: `npm run seed -- 00 && CORPUS_EXTRACTION_MODE=fixture npm run seed -- 01 && npm run seed -- 02 && npm run seed -- 03 && npm run seed -- 04`; e2e uses `workers: 1`.
