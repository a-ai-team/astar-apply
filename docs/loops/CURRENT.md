...
- **Task:** 3 (+9) / 10 — `seed -- 05` derives 6 flashcards + demo progress for e2e-student (idempotent, ran twice)
...
- **Next action:** task 4 question bank (`src/lib/practice/queries.ts`, `/home/practice`, `/home/practice/[slug]`, `QuestionCard`, `recordAttempt`), then 5 palette, 6 flashcards, 7 lesson links, 8 dashboard, 10 e2e. Seeds before e2e: `npm run seed -- 00 && CORPUS_EXTRACTION_MODE=fixture npm run seed -- 01 && npm run seed -- 02 && npm run seed -- 03 && npm run seed -- 04 && npm run seed -- 05`; e2e uses `workers: 1`.
