...
- **Task:** 9 / 10 — `fixtures/eval/grader.jsonl` (40 rows: 10/15/10/5) + `scripts/eval/suites/grader.ts` (Spearman/MAE/empty; skips with `NO API CREDIT`; fixture numbers Spearman 0.921 · MAE 1.00 after one anchor tune) + thresholds
...
- **Next action:** task 10 — `scripts/seed/07-demo-interview.ts`, `e2e/07-interviews.spec.ts`, docs (`TECHNICALS.md` § Mock interviews), retro, PR.
