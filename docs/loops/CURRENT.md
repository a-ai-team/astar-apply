...
- **Task:** 8 / 10 done — `src/lib/pulse/queries.ts`, `DigestView` (take / talking points / anchors / practice Qs / sources, synthetic banner on the fixture digest), `/home/pulse` (latest approved + archive) and `/home/pulse/[week]` (Monday-only, 404 otherwise); next: admin firm editor
...
- **Next action:** `/admin/firms` list + `/admin/firms/[slug]` JSON editor (FirmSchema live validation, status select, unverified badge on `generated`, approve-all-questions action) and `/admin/pulse` (list + approve/reject).
