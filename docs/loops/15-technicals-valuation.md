...
_Status: planned. Protocol: `docs/loops/README.md`. Contracts: `docs/loops/CONTRACTS.md` § Technicals v2. Spec: `docs/research/technicals-v2/15-valuation.md`. Depends on Loop 11 (platform) and Loop 14 (`multiple_matcher`, `ev_bridge` refactor)._
...
- [ ] vitest `src/lib/finance/comps.test.ts` ≥ 15 cases, every spec figure reproduced
...
- [ ] `seed -- 03` idempotent (5 lessons, 31 questions `generated`; week-5 path items resolve), `seed -- 05` derives 27 cards (lens questions skipped), `content:index` adds the chapter's chunks
- [ ] Playwright `e2e/15-valuation.spec.ts` (approves lesson 1 in `beforeAll`, restores after): lesson renders the required blocks; `predict` reveal; football field — toggle precedents off → overlap readout changes; comps mode — untick a peer → median text changes; `?lens=tmt` shows the TMT heading; cheat sheet renders the Rule of 40 formula
...
- [ ] lesson 1 `valuation-methodologies` (validate)
- [ ] lesson 2 `comparable-companies` (+ `order_steps`, `fill_numbers`)
- [ ] lesson 3 `precedent-transactions` (+ `fill_numbers`)
- [ ] lesson 4 `multiples-and-metrics` (+ `fill_numbers`)
- [ ] lesson 5 `choosing-and-presenting`
...
- [ ] `e2e/15-valuation.spec.ts`, `docs/TECHNICALS.md` § Valuation, retro, RUNLOG
