...
`/` redirects to `/home`. The app lives under `/home` (students; `/home` itself is the toolkit landing page — copy in `src/content/home.ts`, roster in `src/content/mentors.ts`) and
