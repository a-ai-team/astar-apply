...
## Launch: landing, billing, entitlements, playbook, SEO (Loop 10)

- **Public pages** (outside the proxy matcher, indexable): `/` (`src/app/page.tsx` — hero, `DemoChat`, curriculum preview from `CURRICULUM`, "Couldn't I just ask AI?" table, placeholder uni strip + testimonials marked `placeholder`, pricing teaser), `/pricing`, `/non-target`, `/privacy`, `/terms` (both `draft`-flagged templates via `src/components/site/legal.tsx`). Shared chrome in `src/components/site/chrome.tsx`.
- **Demo chat**: `POST /api/demo-chat` — unauthenticated, `sources: ["corpus"]` only, fixture composer unless `DEMO_CHAT_MODE=live`, no persistence, `DEMO_CHAT_DAILY_CAP` (3) per salted-hashed IP per UTC day (`src/lib/demo/usage.ts`: in-memory + `increment_demo_usage()` when 0011 is applied; the higher count wins).
- **Plans**: `src/lib/billing/plans.ts` `PLANS` (free £0 / core £4.99 / ai £9.99, feature keys) is the source of truth; `plans` table (0011) mirrors it (`seed -- 10`, no-ops when absent) and holds `stripe_price_id` (`scripts/billing/sync-stripe.ts`).
- **Entitlements**: `src/lib/billing/entitlements.ts` — `can(ent, feature, { isFree })` (free topics pass for everyone), `getEntitlement` (table → default free; dev memory store when 0011 is absent), `setPlan` (service role; webhook + stub). `src/lib/billing/session.ts` `getSessionEntitlement()` is the per-request wrapper. Gates: lesson page (paid topic → title + TOC + `UpgradeCard`), practice bank (free topics only + card), question page, drills/mocks/firm "Practise this" (`ai_drills` / `ai_mocks`), decks (`flashcards_all`), SRS analytics card on `/home/flashcards` (`srs_analytics`).
- **Stripe**: `src/lib/billing/stripe.ts` — `RealStripe` when `STRIPE_SECRET_KEY` is set, else `StripeStub` (checkout → `/billing/success?stub=1&plan=…`, which grants the plan only when `NODE_ENV !== "production"`). Actions `startCheckout` / `openPortal` (`src/app/pricing/actions.ts`, `verifySession` first). `POST /api/stripe/webhook` verifies the signature over the raw body then `handleStripeEvent` (`src/lib/billing/webhook.ts`: `checkout.session.completed`, `customer.subscription.{created,updated,deleted}`, `invoice.payment_failed`) — unit-tested against `fixtures/recorded/stripe/*.json` with `Stripe.webhooks.generateTestHeaderString`.
- **Playbook**: `content/playbook/*.json` (7 original sections; zod-validated by `src/lib/playbook/content.ts`), `Checklist` persists to `playbook_progress` (own rows) when the table exists, always mirrored to `localStorage`.
- **SEO**: root `metadata` (`metadataBase` from `NEXT_PUBLIC_SITE_URL`, title template, OG/Twitter), `src/app/opengraph-image.tsx`, `sitemap.ts` (`PUBLIC_ROUTES`), `robots.ts` (disallows `/home /admin /api /billing /unlock /auth`), JSON-LD via `src/components/site/json-ld.tsx`.
- **Analytics**: `src/lib/analytics/client.ts` `track()` → PostHog (`NEXT_PUBLIC_POSTHOG_KEY`, lazy-loaded, distinct id = user id, no email) + Vercel Analytics custom events; events `signup`, `lesson_complete`, `chat_message`, `checkout_started`, `subscribed`. In dev, events are also pushed to `window.__astarEvents`.
- **Flag**: `PUBLIC_LAUNCH=true` skips gate 1 in `src/proxy.ts` (`isPublicLaunch()`); gate 2 always. See `docs/PRIVATE_AREA.md`.
