...
export type InterviewQuestion = GradeQuestion & { subtopic_slug: string | null; subtopic_title: string | null };
...
export async function getTurns(db: SupabaseClient, interviewId: string): Promise<TurnRow[]> {
  const { data, error } = await db.from("interview_turns").select("id, interview_id, ordinal, question_id, attempt_id, shown_at, answered_at, answer_text, transcript_meta, score, grade, graded_at").eq("interview_id", interviewId).order("ordinal");
  if (error) throw error;
  return (data ?? []) as TurnRow[];
}

/** Approved questions by id with everything the grader and the runner need; unknown/unapproved ids are simply absent. */
export async function getInterviewQuestions(db: SupabaseClient, ids: string[]): Promise<Map<string, InterviewQuestion>> {
  ...
  for (const r of data ?? []) {
    ...
    out.set(r.id as string, {
      ...
      model_answer_md: body.data.model_answer_md, key_points: body.data.key_points, weak_answer_note: body.data.weak_answer_note, numbers: body.data.numbers ?? null,
    });
  }
  return out;
}
